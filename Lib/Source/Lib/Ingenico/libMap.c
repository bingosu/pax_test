...
VOID MemClear(tMtiMap *pMap)
{
	...
	tMtiMem *tpMapStepingStone = NULL;
	...
	if (pMap != NULL && pMap->tpFirstNode != NULL)
	{
		...
		while (tpMapNode != NULL)
		{
			...
			tpMapStepingStone = tpMapNode;
			tpMapNode = tpMapNode->tpNextNode;
			MEM_FREE(tpMapStepingStone);
		}
	}
...
VOID MemFree(tMtiMap *pMap, INT iID)
{
	...
	INT i = 0, isFound = 0, iAllocSize = 0;
	...
	if (pMap != NULL && pMap->tpFirstNode != NULL)
	{
		...
		while (tpMapNode != NULL)
		{
			if (tpMapNode->iID == iID)
			{
				...
				ucpAllocPtr = (UCHAR*)tpMapNode->pAllocAddr;
				for (iAllocSize = 0; iAllocSize < tpMapNode->iSize; iAllocSize++)
				{
					(*ucpAllocPtr++) = 0;
				}
...
VOID *MemAlloc(tMtiMap *pMap, INT iSize, INT iID, INT iType)
{
	...
	for (iAllocSize = 0; iAllocSize < tpMapItem->iSize; iAllocSize++)
	{
		(*ucpAllocPtr++) = 0;
	}
