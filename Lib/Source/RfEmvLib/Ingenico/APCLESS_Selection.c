...
int APCLESS_Selection_GiveInfo (TLV_TREE_NODE * tlvTree, const int explicitSelection)
{
   ...
   unsigned char aidpayWaveElect[] = {0xA0, 0x00, 0x00, 0x00, 0x03, 0x20, 0x10};
   ...
   unsigned char aidpayWaveTTQ[] = {0x36, 0x00, 0x40, 0x00};

   unsigned char aidpayWaveFloorLimit[]= {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
   unsigned char aidpayWaveCvmRequiredLimit[] = {0x00, 0x01, 0x00, 0x00, 0x00, 0x00};
   //unsigned char aidpayWaveTranFloorLimit[]= {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

   	//JSPEEDY //@@WAY JCB Cless
	unsigned char aidJSpeedy[] = {0xA0, 0x00, 0x00, 0x00, 0x65, 0x10, 0x10}; 		   // AID JSpeedy (JCB Contactless) 
	unsigned char aidJSpeedyId[] = {0x00, 0x00, 0x00, 0x02}; 						   // SEPC JSpeedy AID internal identifier = 2
	unsigned char aidJSpeedyKernelToUse[] = {0x00, 0x05};							   // JSpeedy kernel number = 5
	unsigned char aidJSpeedyOptions[] = {0x05, 0x01, 0x00, 0x00};					   // Partial AID & Zero amount + EP allowed
	unsigned char aidJSpeedyFloorLimit[]= {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};		   // JSpeedy Floor Limit
	unsigned char aidJSpeedyCvmRequiredLimit[] = {0x00, 0x01, 0x00, 0x00, 0x00, 0x00};  // JSpeedy CVM Limit
	unsigned char aidJSpeedyTransLimit[]= {0x00, 0x10, 0x00, 0x00, 0x00, 0x00};	   // JSpeedy Transaction Limit
	// 
   result = FALSE;
   ...
   if (tlvTree != NULL)
   {
      ...
      // optional : add floor limits
      // ...
      ///VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_CLESS_TRANSACTION_LIMIT, aidpayWaveTranFloorLimit, sizeof(aidpayWaveTranFloorLimit)));
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_CLESS_CVM_REQUIRED_LIMIT,  aidpayWaveCvmRequiredLimit, sizeof(aidpayWaveCvmRequiredLimit)));
      VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_CLESS_FLOOR_LIMIT, aidpayWaveFloorLimit, sizeof(aidpayWaveFloorLimit)));
      ...
      //  --------- Add payWave AID ---------------
		...
		// TAG_AID_TERMINAL : 0x9F06  -> Visa payWave AID
		VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EMV_AID_TERMINAL, aidpayWaveElect, sizeof(aidpayWaveElect)) != NULL);
		...
		// TAG_AID_TERMINAL : 0x9F06	-> J/SPEEDY AID
		VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EMV_AID_TERMINAL, aidJSpeedy, sizeof(aidJSpeedy)) != NULL);

		// TAG_GENERIC_AID_PROPRIETARY_IDENTIFIER : 0x9F928202 (corresponds to the index for quick search after application selection)
		VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_GENERIC_AID_PROPRIETARY_IDENTIFIER, aidJSpeedyId, sizeof(aidJSpeedyId)) != NULL);

		// TAG_EP_KERNEL_TO_USE : 0x9F928101
		VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_KERNEL_TO_USE, aidJSpeedyKernelToUse, sizeof(aidJSpeedyKernelToUse)));

		// TAG_EP_AID_OPTIONS : 0x9F928100
		VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_AID_OPTIONS, aidJSpeedyOptions, sizeof(aidJSpeedyOptions)));

		// optional : add floor limits
		VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_CLESS_TRANSACTION_LIMIT, aidJSpeedyTransLimit, sizeof(aidJSpeedyTransLimit)));
		VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_CLESS_CVM_REQUIRED_LIMIT, aidJSpeedyCvmRequiredLimit, sizeof(aidJSpeedyCvmRequiredLimit)));
		VERIFY(TlvTree_AddChild(tmpAidInfo, TAG_EP_CLESS_FLOOR_LIMIT, aidJSpeedyFloorLimit, sizeof(aidJSpeedyFloorLimit)));
		//=============================================== //@@WAY JCB Cless ===============================================
...
int APCLESS_Selection_GuiCustomisation(T_SHARED_DATA_STRUCT* dataStruct)
{
   ...
   if (ret == STATUS_SHARED_EXCHANGE_OK) // If tag found
   {
      ...
      dmsg("**customStep = %d", customStep);
      g_iRetGuiStep = customStep;
      switch (customStep)
      // Steps to customize
      {
