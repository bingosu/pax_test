...
unsigned char __APCLESS_ParamTrn_transactionDate[3] = {0}; // YYMMDD
unsigned char __APCLESS_ParamTrn_transactionTime[3] = {0}; // HHMMSS
unsigned char __APCLESS_ParamTrn_isAmountPresent = TRUE;
unsigned char __APCLESS_ParamTrn_transactionAmountBin[4] = {0};
unsigned char __APCLESS_ParamTrn_transactionAmountBcd[6] = {0};
unsigned char __APCLESS_ParamTrn_transactionCurrencyExponent = 2; // Currency exponent is 2
unsigned char __APCLESS_ParamTrn_transactionCurrencyCode[2] = {0x03,0x60};  // euro
unsigned char __APCLESS_ParamTrn_transactionType = APCLESS_TRANSACTION_TYPE_DEBIT; //purchase
unsigned long __APCLESS_ParamTrn_transactionSequenceCounter = 1;   // Application Transaction Sequence Counter

unsigned long __APCLESS_ParamTrn_ulAmount = 0;
 ...
 */
void APCLESS_ParamTrn_SetAmount(unsigned long ulAmount)
{
	GTL_Convert_UllToDcdNumber(ulAmount, __APCLESS_ParamTrn_transactionAmountBcd, sizeof(__APCLESS_ParamTrn_transactionAmountBcd));
	GTL_Convert_UlToBinNumber(ulAmount, __APCLESS_ParamTrn_transactionAmountBin, sizeof(__APCLESS_ParamTrn_transactionAmountBin));
	__APCLESS_ParamTrn_isAmountPresent = TRUE;
	__APCLESS_ParamTrn_ulAmount = ulAmount;
}
...
const unsigned char* APCLESS_ParamTrn_GetAmountBin(void)
{
   if(__APCLESS_ParamTrn_isAmountPresent == TRUE)
   {
	   dbuf("__APCLESS_ParamTrn_transactionAmountBin", __APCLESS_ParamTrn_transactionAmountBin, 4);
...
void APCLESS_ParamTrn_SetDateTime(void)
{
   ...
   if(0 == Telium_Read_date(&tDate))
   {
      __APCLESS_ParamTrn_transactionDate[0] = (unsigned char)(((tDate.year[0] - '0') << 4) | (tDate.year[1] - '0'));
      __APCLESS_ParamTrn_transactionDate[1] = (unsigned char)(((tDate.month[0] - '0') << 4) | (tDate.month[1] - '0'));
      __APCLESS_ParamTrn_transactionDate[2] = (unsigned char)(((tDate.day[0] - '0') << 4) | (tDate.day[1] - '0'));

      __APCLESS_ParamTrn_transactionTime[0] = (unsigned char)(((tDate.hour[0] - '0') << 4) | (tDate.hour[1] - '0'));
      __APCLESS_ParamTrn_transactionTime[1] = (unsigned char)(((tDate.minute[0] - '0') << 4) | (tDate.minute[1] - '0'));
      __APCLESS_ParamTrn_transactionTime[2] = (unsigned char)(((tDate.second[0] - '0') << 4) | (tDate.second[1] - '0'));
   }
   else
   {
      memset(__APCLESS_ParamTrn_transactionDate, 0 , sizeof(__APCLESS_ParamTrn_transactionDate));
